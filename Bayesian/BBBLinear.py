...
class BBBLinear(ModuleWrapper):
    def __init__(self, in_features, out_features, bias=True, priors=None):
        ...
        self.W_mu = Parameter(th.empty((out_features, in_features), device=self.device))
        self.W_rho = Parameter(th.empty((out_features, in_features), device=self.device))

        if self.use_bias:
            self.bias_mu = Parameter(th.empty((out_features), device=self.device))
            self.bias_rho = Parameter(th.empty((out_features), device=self.device))
        else:
